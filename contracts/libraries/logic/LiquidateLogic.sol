...
import {IxdToken} from "../../interfaces/IxdToken.sol";
...
library LiquidateLogic {
  ...
  function executeAuction(
    ILendPoolAddressProvider addressesProvider,
    ...
    IERC20(loanData.reserveAsset).safeTransferFrom(vars.initiator, address(this), params.bidPrice);

    // transfer (return back) last bid price amount to previous bidder from lend pool
    if (loanData.bidderAddress != address(0)) {
      IERC20(loanData.reserveAsset).safeTransfer(loanData.bidderAddress, loanData.bidPrice);
    }
  ...
  function executeRedeem(
    ...
    IxdToken(reserveData.debtTokenAddress).burn(loanData.borrower, vars.repayAmount, reserveData.variableBorrowIndex);
    ...
    IERC20(loanData.reserveAsset).safeTransferFrom(
      vars.initiator,
  ...
  function executeLiquidate(
    ...
    IxdToken(reserveData.debtTokenAddress).burn(
      loanData.borrower,
      vars.borrowAmount,
      reserveData.variableBorrowIndex
    );
    ...
    if (vars.extraDebtAmount > 0) {
      IERC20(loanData.reserveAsset).safeTransferFrom(vars.initiator, address(this), vars.extraDebtAmount);
    }

    // transfer borrow amount from lend pool to bToken, repay debt
    IERC20(loanData.reserveAsset).safeTransfer(reserveData.bTokenAddress, vars.borrowAmount);

    // transfer remain amount to borrower
    if (vars.remainAmount > 0) {
      IERC20(loanData.reserveAsset).safeTransfer(loanData.borrower, vars.remainAmount);
    }

    // transfer erc721 to bidder
    IERC721(loanData.nftAsset).safeTransferFrom(address(this), loanData.bidderAddress, params.nftTokenId);
