...
library BorrowLogic {
  ...
  function _borrow(
    ...
    vars.totalSupply = IERC721Enumerable(params.nftAsset).totalSupply();
    ...
    IxdToken(reserveData.debtTokenAddress).mint(
      vars.initiator,
      params.onBehalfOf,
      params.amount,
      reserveData.variableBorrowIndex
    ...
    IlolToken(reserveData.bTokenAddress).transferUnderlyingTo(vars.initiator, params.amount);
  ...
  function _repay(
    ...
    if (!vars.isUpdate) {
      IERC721(loanData.nftAsset).safeTransferFrom(address(this), loanData.borrower, params.nftTokenId);
    }
