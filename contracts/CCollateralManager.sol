...
contract CollateralManager {
    ...
    function updateAllLiquidatableCollateral() external {
        address[] memory borrowerList = iPool.getBorrowerList();
    ...
    function addCollateral(address borrower, address collectionAddress, uint256 tokenId) public onlyPortal {
        ...
        uint len = iPool.getBorrowerList().length;
        iPool.addBorrower(borrower);
        uint newLen = iPool.getBorrowerList().length;
        if (len != newLen) {
