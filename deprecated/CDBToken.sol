...
contract DBToken is ERC20 {
    ...
    bool public paused; // Pausing mechanism for security
    ...
    // Get the amount of DB tokens a specific holder has
    function getAmount(address holder) external view returns (uint256) {
        return balanceOf(holder);
    }

    // Get the total active DB tokens in circulation
    ...
    function _removeHolder(address holder) private {
        ...
        if (index != lastIndex) {
            ...
            holderIndex[lastHolder] = index; // Update index of the last holder
        }
